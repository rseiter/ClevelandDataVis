...
Trellis (Lattice in R) graphics examples from [Visualizing Data](http://www.stat.purdue.edu/~wsc/visualizing.html) by William Cleveland.  [Amazon](http://www.amazon.com/Visualizing-Data-William-S-Cleveland/dp/0963488406) 
...
```{r chapter3NotImplemented, eval=FALSE, echo=FALSE}
book.3.2 <- 
function()
	xyplot(cp.ratio ~ area, 
		data = ganglion,
...
```{r chapter4NotImplemented, eval=FALSE, echo=FALSE}
book.4.2 <- 
function()
	splom(~rubber[, 1:3], 
		panel = function(x, y) {
...
```{r chapter5NotImplemented, eval=FALSE, echo=FALSE}
book.5.2 <- 
function()
{
...
function()
{
  ...
  organ <- reorder(organ,weight,median)
	bwplot(organ ~ weight,
		aspect = 0.3,
...
# 	new.livestock <- reorder.factor(livestock.type, logcount, median)
  new.country <- reorder(country, logcount, median)
	new.livestock <- reorder(livestock.type, logcount, median)
...
# ```
# 
# ```{r chapter6NotImplemented, eval=FALSE, echo=FALSE}
book.6.2 <- 
function()
{
	attach(livestock)
...
# requires map library -- with the world database (available from statlib)
  ...
  europe <- map("world", xlim = xlim, ylim = ylim, plot = F)
	logcount <- logb(count,2)
	wt <- rep(1,length(logcount))
	for(i in 1:10){
	        liv.lm <- lm(logcount~livestock.type+country,weights=wt)
...
R and S-Plus differences
------------------------

reorder.factor in S-Plus becomes reorder in R: https://stat.ethz.ch/pipermail/r-devel/2004-September/030708.html

S-Plus wt.bisquare becomes psi.bisquare in R MASS package.

map handling differs
